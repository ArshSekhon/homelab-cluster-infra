...
NODE="node-1.cluster.arpa"
CONFIG="bootstrap/cloud-init/node-1.yaml"

echo "Copying cloud-init config to $NODE..."
scp "$CONFIG" "${NODE}:/tmp/node-1.yaml"

echo "Injecting config and triggering re-run..."
ssh -o RequestTTY=force "$NODE" "sudo mkdir -p /var/lib/cloud/seed/nocloud && \
  sudo cp /tmp/node-1.yaml /var/lib/cloud/seed/nocloud/user-data && \
